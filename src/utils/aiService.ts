  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        // Removed response_format for maximum model compatibility
      }),
    });
    ...
    if (!content.html || !content.css) {
      throw new Error('The AI failed to generate both HTML and CSS. Please try a different prompt.');
    }
    ...
    console.error('🚨 AI Service Crash:', error);
    throw error;
  }
};
